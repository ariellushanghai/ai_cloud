        ...
        el-main.project-details-main
            ...
            el-dialog.dialog-build-image(:visible.sync='dialog_add_training_visible', width='61.8%', append-to-body='', modal-append-to-body='', lock-scroll='', :before-close="handleCloseDialogBuildImage", :show-close='false', :close-on-click-modal='false', :close-on-press-escape='false')
                ...
                .form-container(v-loading='isBuildingImage')
                    // 第1步: 输入训练名
                    el-form(v-show='at_step_add_training === 0', :model='form_naming_train', :rules='rules_naming_train', ref='form_naming_train', element-loading-background='rgba(255, 255, 255, 0.1)', :disabled='isNamingTrain', :status-icon='true', label-position='left', label-width='200px', size='small')
                        |
                        ...
                        el-form-item.input-dir(label='训练目录')
                            el-input(v-model.trim='form_naming_train.rprojectName', disabled='')
                                |
                                template(slot="prepend") {{prependProjectDir}}
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            disableBtnBuildImage() {
                if (this.isTransformingFile || this.isBuildingImage) {
                    ...
                    console.log(`L311`)
                    return true;
                } else if (this.form_build_image.rprojectName.length < 3 || !this.form_build_image.imageName) {
                    console.log(`L314`)
                    return true;
                } else {
                    return false;
        ...
        methods: {
            ...
            postFormNamingTrain(raw_form_data) {
                let self = this;
